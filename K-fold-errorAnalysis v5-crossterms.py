# -*- coding: utf-8 -*-
"""
CS229 project analysis with crossterms
"""
...
df_orig = df
df_cross = df[featurecols]
allpairs= combinations_with_replacement(featurecols, 2)
for pair in allpairs:
    paircol = pair[0]+pair[1]
    df_cross[paircol]=df_cross[pair[0]]*df_cross[pair[1]]
X = df_cross
scaled_X = StandardScaler().fit_transform(X.values)
X = pd.DataFrame(scaled_X, columns=X.columns)
...
X = pd.DataFrame(X,columns=pcacols)
...
#    plt.ylim(0,1)
    ...
    plt.savefig(path+'\Figures_1\\'+name+'_Hyperparam.png')
    plt.show()
    plt.close()
...
#plot y_pred vs y_actual'''
    ...
    plt.text(.99, .01, keyword+'='+str(bestparam)+ r'$\mathregular{, R^2 =}$'+str(classifierscore),
             ...
             horizontalalignment='right')
    plt.tight_layout(0.5)
    plt.savefig(path+'\Figures_1\\'+name+'_pred_vs_act.png')
    plt.show()
    plt.close()
...
for name, estimator, keyword, hyperparams in regression_algorithms: 
    ...
    plt.ylim(0,1)
    plt.tight_layout(0.5)
    plt.savefig(path+'\Figures_1\\'+name+'_feature_size.png')
    plt.show()
    plt.close()
...
for name, estimator, keyword, hyperparams in regression_algorithms: 
    ...
    plt.tight_layout(0.5)
    plt.ylim(0,1)
    plt.savefig(path+'\Figures_1\\'+name+'_sample_size.png')
    plt.show()
    plt.close()
